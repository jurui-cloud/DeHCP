...
class PointPillarDepthLoss(PointPillarLoss):
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        # ==================== [FG-SSPF: 计算额外损失] ====================
        # 2. 计算共享分支辅助 Loss (L_det_shared)
        if 'cls_preds_shared' in output_dict:
            # 利用 suffix 后门，直接复用父类的检测 Loss 代码！
            aux_loss = super().forward(output_dict, target_dict, suffix="_shared")
            total_loss += aux_loss * 1.0  # 辅助头权重设为 1.0
            self.loss_dict.update({'aux_loss': aux_loss.item()})
            
        # 3. 计算门控稀疏正则化 (L_gate)
        if 'alpha' in output_dict:
            alpha = output_dict['alpha']
            gate_loss = alpha.abs().mean()  # L1 范数
            total_loss += gate_loss * 0.05  # 正则化权重稍微给小一点，防止把特有分支拍死
            self.loss_dict.update({'gate_loss': gate_loss.item()})
        # =================================================================
    ...
    def logging(self, epoch, batch_id, batch_len, writer = None, suffix=""):
        ...
        aux_loss = self.loss_dict.get('aux_loss', 0)
        gate_loss = self.loss_dict.get('gate_loss', 0)


        print("[epoch %d][%d/%d]%s || Loss: %.4f || Conf Loss: %.4f"
              " || Loc Loss: %.4f || Dir Loss: %.4f || IoU Loss: %.4f || Depth Loss: %.4f"
              " || Aux Loss: %.4f || Gate Loss: %.4f" % (
        ...
        if not writer is None:
            ...
            writer.add_scalar('Depth_loss' + suffix, depth_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Aux_loss' + suffix, aux_loss,
                            epoch*batch_len + batch_id)
            writer.add_scalar('Gate_loss' + suffix, gate_loss,
                            epoch*batch_len + batch_id)
